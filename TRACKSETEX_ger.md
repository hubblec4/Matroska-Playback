...
Es wird ein `Tag` benutzt mit je einem `SimpleTag` (im XML nur `Simple`) für jeden TrackSet. Es wird kein `Target` Element verwendet. Im `TagName` Element wird das Schlüsselwort "TRACKSETEX" eingeben und in das `String` Element werden die Paramter gespeichert.
