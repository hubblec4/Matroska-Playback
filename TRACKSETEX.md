...
A `Tag` is used with one `SimpleTag` (in the XML only `Simple`) for each TrackSet. No `Target` element is used. In the `TagName` element the keyword "TRACKSETEX" is entered and in the `String` element the parameters are stored.
